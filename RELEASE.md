...
| Environment | Previous version | Current version (100%) |
| --- | --- | --- |
| Preview | `ea4df593-b0c7-4c7f-8faf-62f4276e16d4` | `7131c9bb-1ae6-40a3-8c70-28ad13c3eedd` |
| Production | `91828030-89c5-40aa-bf09-e67ee1064b78` | `17980135-9482-492a-8dd8-a97d6f31434d` |

- **Behavior:** all failed (HTTP 400+) and slow API/discovery responses emit safe completion logs; routine responses are sampled at 10%. Logs include `sampleRate`, request/release IDs, route pattern, status and duration. `/api/health` returns 200/ok after a minimal D1 query, or 503/degraded on failure/timeout, without application-table reads. Cloudflare head sampling remains 100%.
- **Validation:** full `bun run check` passed with 308 tests before release. Fresh preview/production builds and exact-config dry-runs passed. Each environment passed release smoke and 15 public checks, including healthy/no-store response, request IDs, authentication boundaries, security headers and exact HTML/four referenced assets. One header-filtered unauthenticated test request per environment verified the completion field allowlist, request/release correlation, release tag and retained failure sample rate 1. No production failure was injected. All 251 source hashes and client assets match across frozen releases, each with 15 artifact hashes.
- **Configuration:** preserved all 13 live variables, six inherited secret bindings, owner-only allowlists, D1/R2/Images/Browser/Workflow resources, runtime/static routing, observability and domains. The sole binding addition is `CF_VERSION_METADATA`. No secret values were read. Wrangler's existing warning about API-returned `observability.logs.redact_query_string` remains; post-deployment configuration comparison passes. No schema/dependency changes, pending migrations or catalog reads/writes. Recovery bookmarks were captured read-only.
- **Alerts:** $5 and $10 usage-spend alerts are saved for the chosen email; existing $2 preserved. These account-wide alerts do not cap spending. The website monitor request was already submitted once; a second request for the now-live health URL was submitted once after production verification. UptimeRobot returned HTTP 200, which does not establish delivery or activation. On 2026-09-21 the user confirmed activation and signed in; the dashboard contained only the website monitor. Created the missing health monitor once directly. The dashboard now verifies both monitors Up, every five minutes, with email contacts and account Up/Down notifications enabled. IDs: website `804041104`, health `804042598`. The user subsequently confirmed completing the downtime/recovery email-delivery test on 2026-09-21; this is recorded as user confirmation, not an independent inbox check. Do not reactivate the old health setup email or create duplicates. No paid monitoring subscription or domain-wide traffic policy was added.
- **Recovery/evidence:** `/tmp/wantkit-monitoring-release-a0f23a2/` contains private frozen configurations, manifests, migration checks, bookmarks, one-shot deployment guards and verification results. Previous Worker versions above are the migration-free rollback targets; deliberately pause/retarget the health monitor if rolling back to a version without that endpoint. Do not rerun completed deploy scripts. Setup evidence is under `/tmp/wantkit-monitoring-20260920/`; do not resend monitor requests automatically.

Only HANDOFF/MONITORING/PROJECT/RELEASE notes changed after deployment. This documentation closeout is prepared for the exact staged-tree commit approval gate. No further deployment is required. See [MONITORING.md](./MONITORING.md) for verification evidence and investigation steps.
