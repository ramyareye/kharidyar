...
1. **Finish UI review/release when requested.** UI changes are local and unstaged, alongside the earlier local-runner changes. The quality gate regenerated production/default deployment artifacts; they do not contain the private preview owner override. A future approved preview deployment needs a fresh preview build and deliberate MCP/local-runner flag restoration.
2. **Chat actions through ChatGPT/Claude first.** The user ultimately wants both assistant chats and WantKit chat. Current MCP remains read-only; local Codex remains a bounded research runner. Add authenticated write tools over existing services, then validate the two actual clients. Include friendly reconnect guidance in this connector task. Keep the paid OpenAI API deferred.
